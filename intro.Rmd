...
value to an argument, called a parameter (e.g. `function_name(argument1 = parameter1)`),
lets the function perform its action with that parameter value. Let's make a
simple example by doing a sum of two values, plus an extra third that has the
default value of 0:
...
a parameter, while the third has a default so doesn't need a parameter. Let's
add 2 with 2. There are two ways to do it, using positional parameters and named
parameters.
